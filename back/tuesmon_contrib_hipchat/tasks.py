...
from tuesmon.base.api.renderers import UnicodeJSONRenderer
from tuesmon.base.utils.db import get_typename_for_model_instance
from tuesmon.celery import app
...
def change_hipchathook(url, notify, notify_config, obj, change):
    ...
    template_change = loader.get_template('tuesmon_contrib_hipchat/change.jinja')
    ...
    # Get markdown fields
    if change.diff:
        template_field = loader.get_template('tuesmon_contrib_hipchat/field-diff.jinja')
...
def create_hipchathook(url, notify, notify_config, obj):
    ...
    template = loader.get_template('tuesmon_contrib_hipchat/create.jinja')
...
def delete_hipchathook(url, notify, notify_config, obj):
    ...
    template = loader.get_template('tuesmon_contrib_hipchat/delete.jinja')
    context = Context({"obj": obj, "obj_type": obj_type})
